...
'''#Everyone picks 
workbook = xlsxwriter.Workbook('Everyones picks.xlsx')
worksheet = workbook.add_worksheet()
row,col=0,0
for b1 in brackets:
    col=0
    
    for i, s2 in enumerate(brackets[b1]):
        if False:
            if s2 == 'Patriots': formatpick = format_pats
            if s2 == 'Steelers': formatpick = format_steelers
            if s2 == 'Falcons': formatpick = format_falcons
            if s2 == 'Panthers': formatpick = format_panthers
            if s2 == 'Jaguars': formatpick = format_jags
            if s2 == 'Vikings': formatpick = format_viks
            if s2 == 'Saints': formatpick = format_saints
            if s2 == 'Eagles': formatpick = format_eags
            if s2 == 'Titans': formatpick = format_titans
            if s2 == 'Chiefs': formatpick = format_chiefs
            if s2 == 'Colts' : formatpick = format_colts
            if s2 == 'Rams' : formatpick = format_rams
            if s2 == 'Cowboys' : formatpick = format_cowboys
            if s2 == 'Ravens' : formatpick = format_ravens
            if s2 == 'Bears' : formatpick = format_bears
            if s2 == 'Chargers' : formatpick = format_chargers
            if s2 == 'Seahawks' : formatpick = format_seahawks
            if s2 == 'Texans' : formatpick = format_texans

        if i==0: worksheet.write(row,col,b1)
        print(i,b1)
        worksheet.write(row,col,s2)
        print(s2)
        print(row,col,h1)
        col = col + 1
    row = row + 1
